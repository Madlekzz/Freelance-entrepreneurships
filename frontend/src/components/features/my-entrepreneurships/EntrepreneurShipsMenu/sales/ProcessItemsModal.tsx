...
export default function ProcessItemsModal({
  isOpen,
  onClose,
  onConfirm,
  isLoading,
  ...
  const processableItems = saleItems.filter(
    (item) => !item.refunded && !item.entrepreneur_processed,
  );
  const allSelected =
    processableItems.length > 0 &&
    selectedIds.length === processableItems.length;
  const processTotal = saleItems
  ...
  const handleToggleAll = () => {
    if (allSelected) {
      ...
      setSelectedIds(processableItems.map((item) => item.id));
    }
  };
          ...
          <>
            ...
            <div className="p-6 space-y-3">
              <p className="text-sm text-gray-600">
                Selecciona los productos que ya fueron pagados de esta venta:
              </p>
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {saleItems.map((item) => {
                  const isProcessed = item.entrepreneur_processed && !item.refunded;
                  const isRefunded = item.refunded;
                  const isDisabled = isProcessed || isRefunded;
                  return (
                    <label
                      key={item.id}
                      className={`flex items-center gap-3 p-3 border rounded-xl transition-colors ${
                        isDisabled
                          ? isRefunded
                            ...
                            : "border-emerald-100 bg-emerald-50/50 cursor-not-allowed"
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isProcessed && (
                            <span className="text-[9px] font-bold text-emerald-500 bg-emerald-100 px-1.5 py-0.5 rounded shrink-0">
                              PROCESADO
                            </span>
                          )}
                        </div>
            ...
            <div className="p-6 pt-4 border-t border-gray-100 flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-400">A procesar</p>
                <p className="text-xl font-black text-primary">
                  {formatCurrency(processTotal)}
                </p>
              </div>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2.5 text-sm font-bold text-white bg-primary rounded-xl hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer"
                >
                  Continuar
                </button>
              </div>
          ...
          <>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4">
                <p className="text-sm font-medium text-primary">
                  ¿Estás seguro de marcar como pagados los siguientes productos?
                </p>
              </div>
            ...
            <div className="p-6 pt-4 border-t border-gray-100 flex justify-end gap-2">
              ...
              <button
                ...
                disabled={isLoading}
                className="px-5 py-2.5 text-sm font-bold text-white bg-primary rounded-xl hover:bg-primary/90 transition-colors cursor-pointer disabled:opacity-50 flex items-center gap-2"
              >
                {isLoading ? (
