  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Error al obtener datos de pago";
...
export async function upsertPaymentData(req: Request, res: Response) {
  ...
  try {
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Error al guardar datos de pago";
...
export async function getBatchPaymentData(req: Request, res: Response) {
  try {
    ...
    if (!Array.isArray(product_ids) || product_ids.length === 0) {
      return res.status(400).json({ error: "Se requiere un array de product_ids" });
    }

    if (product_ids.length > 50) {
      return res.status(400).json({ error: "Máximo 50 productos por consulta" });
    }
    ...
    if (prodError) {
      console.error("[getBatchPaymentData] Error fetching products:", prodError);
    ...
    const entrepreneurshipIds = [...new Set(products.map((p) => p.entrepreneurship_id))];
    ...
    if (entError) {
      console.error("[getBatchPaymentData] Error fetching entrepreneurships:", entError);
      return res.status(500).json({ error: "Error al obtener datos de pago" });
    }
    ...
    if (payError) {
      console.error("[getBatchPaymentData] Error fetching payment data:", payError);
      return res.status(500).json({ error: "Error al obtener datos de pago" });
    }
